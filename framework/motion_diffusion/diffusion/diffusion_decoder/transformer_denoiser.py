        ...
        if self.task == "offline":
            ...
            for emb in emb_latent_list:
                if emb.shape[0] == mask_u.shape[-1]:
                    ...
                    mask_other = torch.zeros(size=(time_embed.shape[1], emb.shape[0]),
                                             dtype=torch.bool).to(device)
                    memory_key_padding_mask = torch.cat((memory_key_padding_mask, mask_other), dim=-1)
            ...
            if motion_length is not None:  # during test stage
                ...
                for emb in emb_latent_list:
                    if emb.shape[0] == mask_u.shape[-1]:
                        ...
                        # 处理 speaker_future_emotion_prediction (10帧) 等未匹配的 emb
                        mask_other = torch.zeros(size=(time_embed.shape[1], emb.shape[0]),
                                                 dtype=torch.bool).to(device)
                        memory_key_padding_mask = torch.cat((memory_key_padding_mask, mask_other), dim=-1)
