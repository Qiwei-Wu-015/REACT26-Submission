...
from framework.motion_diffusion.diffusion.gaussian_diffusion import PriorLatentDiffusion, DecoderLatentDiffusion, \
    FlowMatchingDecoder
from framework.motion_diffusion.diffusion.resample import UniformSampler, ContinuousTimeSampler
...
class DecoderLatentMatcher(BaseLatentModel):
    def __init__(self,
        ...
        # Initialize SpeakerEmotionPredictor (optional)
        self.speaker_emotion_predictor = None
        if self.use_speaker_predictor:
            self.speaker_emotion_predictor = SpeakerEmotionPredictor(
                ...
                output_dim=self.emotion_dim, future_frames=10
            )

        # ── Flow Matching vs DDPM dispatch ──
        diffusion_type = conf.scheduler.get("diffusion_type", "ddpm")
        if diffusion_type == "flow_matching":
            self.decoder_diffusion = FlowMatchingDecoder(
                conf.scheduler,
                conf.scheduler.num_train_timesteps,
                conf.scheduler.get("num_inference_timesteps", 20),
            )
            self.schedule_sampler = ContinuousTimeSampler(diffusion=self.decoder_diffusion)
        else:
    ...
    def _forward(
        ...
        # Optional: predict future speaker emotion
        pred_future_emotion = None
        if self.speaker_emotion_predictor is not None:
            pred_future_emotion = self.speaker_emotion_predictor(
                speaker_emotion_input, speaker_audio_input, speaker_3dmm_input
            )
        ...
        with torch.no_grad():
          ...
          # freeze latent RNN_VAE embedder to extract speaker latent embedding
            ...
            # Pass predicted future emotion to model_kwargs
            speaker_future_emotion_prediction = None
            if pred_future_emotion is not None:
        ...
        if self.stage == "test":
            ...
            timesteps = t.long() if not t.is_floating_point() else t
        ...
        # Attach speaker future emotion prediction to outputs
        if pred_future_emotion is not None:
...
class SpeakerEmotionPredictor(nn.Module):
    """
    Predict future speaker emotion frames from past speaker audio/emotion/3dmm.
    Uses GRU to encode temporal context, then predicts next N frames.
    """

    def __init__(self, emotion_dim=25, audio_dim=768, _3dmm_dim=58, hidden_dim=256,
                 num_layers=2, output_dim=25, future_frames=10):
        ...
        self.fc = nn.Linear(hidden_dim, output_dim * future_frames)
    ...
    def forward(self, emotion, audio, _3dmm):
        ...
        prediction = prediction.view(-1, self.future_frames, self.output_dim)
        last_input_frame = emotion[:, -1, :].unsqueeze(1)
        prediction = prediction + last_input_frame
        return prediction
